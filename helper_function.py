...
def add_to_user_data_file(username,option):
    """function to add the title (name) of randomly chosen animal 
    to user's animals/correctlyGuessed/passed list in user data file"""
    # # Open the JSON file for reading
    # with open("data/user_data.json", "r", encoding='utf-8') as jsonFile: 
    #     # Read the JSON into the buffer
    #     data = json.load(jsonFile) 
    ...
    with open("data/user_data.json", "w", encoding='utf-8') as jsonFile:
        for i in data:
            #if entry is found matching current username 
            #then add to animals or correctlyGuessed or passed list depending on option parameter passed
...
def update_user_data_file(username, score=0):
    ...
    or insert user entry into file if user does not exist yet"""
    with open("data/user_data.json", "r", encoding='utf-8') as jsonFile:
        try: 
            data = json.load(jsonFile)
            print(data)
        except ValueError: 
            #set dummy data to avoid value error later on caused by empty json file
            data = [{"username": "username","score": 0, "animals":[], "correctlyGuessed":[], "passed":[]}]
...
def get_current_user_game_history(username, option):
    ...
    (already asked animal list, correctly guessed list and passed list)"""
    with open("data/user_data.json", "r", encoding='utf-8') as jsonFile:
        data = json.load(jsonFile)
        game_history = []
...
def get_leaderboard():
    ...
    with open("data/user_data.json", "r", encoding='utf-8') as jsonFile: # Open the JSON file for reading
        try: 
            ...
            #set dummy data to avoid value error
